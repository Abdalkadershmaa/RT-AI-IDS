...
class JsonKeyLoad:
    """Outcome of reading JSON stored under a broker-managed string key (e.g. predict poll cache).

    * ``payload is None`` and ``corrupt`` is False — key missing or expired.
    * ``payload is None`` and ``corrupt`` is True — key present but JSON invalid.
    * ``payload`` is a dict — successful decode.
    """
