...
| --- | --- | --- |
| B403 import pickle / dill | `services/inference/model_service.py:25-28` | Acknowledged with `# nosec B403`. We never deserialize untrusted input — only the operator-controlled `models/` directory. Provenance documented in `models/README.md`. |
| B301 pickle.load / dill.load | `services/inference/model_service.py:113,150` | Same trust boundary. `# nosec B301` with rationale comment. |
