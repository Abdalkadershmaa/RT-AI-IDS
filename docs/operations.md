...
cd RT-AI-IDS
make bootstrap            # generate .env with random secrets, including DB/Redis passwords
...
For a local TLS-terminated front door, start the optional proxy profile:

```bash
docker compose --profile proxy up -d proxy
```

It listens on `https://localhost:8443` and generates a self-signed cert on
first start.
