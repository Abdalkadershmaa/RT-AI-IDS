...
def test_redis_url_defaults_to_password_auth(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ENVIRONMENT", "development")
    monkeypatch.setenv("SECRET_KEY", "dev")
    monkeypatch.setenv("JWT_SECRET_KEY", "dev")
    monkeypatch.setenv("ADMIN_PASSWORD", "dev")
    monkeypatch.delenv("REDIS_URL", raising=False)
    monkeypatch.setenv("REDIS_PASSWORD", "s3cr3t-redis")

    settings = reload_settings()

    assert settings.redis_url == "redis://:s3cr3t-redis@redis:6379/0"
