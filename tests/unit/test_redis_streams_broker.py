...
def test_store_and_load_result() -> None:
    ...
    loaded = broker.load_result("predict_results:abc")
    assert loaded == JsonKeyLoad(payload={"job_id": "abc", "status": "completed"})
    assert broker.load_result("missing") == JsonKeyLoad()


def test_load_result_detects_corrupt_json() -> None:
    from shared.broker import JsonKeyLoad

    broker, fake = _broker()
    fake.set("predict_results:bad", "not-json{")
    assert broker.load_result("predict_results:bad") == JsonKeyLoad(corrupt=True)


def test_load_result_non_object_json_is_corrupt() -> None:
    from shared.broker import JsonKeyLoad

    broker, fake = _broker()
    fake.set("predict_results:arr", "[1,2,3]")
    assert broker.load_result("predict_results:arr") == JsonKeyLoad(corrupt=True)
