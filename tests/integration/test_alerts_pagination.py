...
def test_risk_label_filter_returns_matching_rows(api_client) -> None:
    token = _get_token(api_client)
    headers = {"Authorization": f"Bearer {token}"}
    ...
    with session_scope() as session:
        ...
        session.add(
            AttackLog(
                ...
                risk_score=0.9,
                rationale=["ml_model_flagged_flow"],
                created_at=datetime(2026, 4, 26, 12, 0, 1, tzinfo=UTC),
            )
        )

    response = api_client.get("/api/v1/alerts?risk_label=high", headers=headers)
    assert response.status_code == 200
    body = response.get_json()
    assert len(body) == 1
    assert body[0]["risk_label"] == "high"
