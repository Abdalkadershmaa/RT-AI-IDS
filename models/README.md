...
3. Add a SHA-256 manifest as `manifest.sha256.json` so deployment can verify integrity before any pickle / dill / joblib deserialization.
...
the inference worker to fail fast at startup. Every load is preceded by SHA-256
verification against `manifest.sha256.json`.
