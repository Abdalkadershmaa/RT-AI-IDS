...
bootstrap:
	...
	POSTGRES_PW=$$($(PYTHON) -c "import secrets; print(secrets.token_urlsafe(24))"); \
	REDIS_PW=$$($(PYTHON) -c "import secrets; print(secrets.token_urlsafe(24))"); \
	...
	sed -i.bak "s|^DATABASE_URL=.*|DATABASE_URL=postgresql+psycopg2://ids:$$POSTGRES_PW@db:5432/ids|" .env && \
	sed -i.bak "s|^REDIS_URL=.*|REDIS_URL=redis://:$$REDIS_PW@redis:6379/0|" .env && \
	rm -f .env.bak
	@echo ".env generated with random SECRET_KEY, JWT_SECRET_KEY, ADMIN_PASSWORD, POSTGRES_PASSWORD, REDIS_PASSWORD."
